...
$list = ngelist(".");
echo '<h1 align="center">Backdoor Scanner</h1>';
echo '<h3 align="center"><a href="https://github.com/jackxcode/Backdoor_Scanner">./J4CK</a></h3>';
foreach ($list as $value) {
    if (is_file($value)) {
        $string = baca($value);
        $cek    = ngecek($string);
        if (empty($cek)) {
            $kata = '<p style="color: red;">'. $value .' => Bukan Shell</p><hr>';
			echo $kata;
        } else if(preg_match("/, /", $cek)) {
            $kata = '<p style="color: green;">'. $value .' => Found ('. $cek .')</p><hr>';
			echo $kata;
			$fp = fopen('resultss.html', 'a');
			fwrite($fp, $kata."\n");
			fclose($fp);
